...
const InvoiceHeader = () => {
  ...
  return (
    ...
    <div className="flex justify-between items-center py-4 px-6 bg-white shadow-sm">    
      <div>
        <h1 className="font-bold text-3xl  text-primaryBlack">Invoice - 1023494 - 2304</h1>
        <p className="text-base font-normal text-primaryGrey mb-6">View the details and activity of this invoice</p>
         {/* Partial Payment button */}
         <button 
                  className=" tracking-[1px] bg-transparent font-medium border border-[#E3E6EF] text-primary rounded-full px-4 py-3 text-sm">
          Partial Payment
        </button>
      </div>
    ...
    <div className='mb-6 flex gap-4 items-center p-4 border border-[#E3E6EF] rounded-3xl'>
        <p className='uppercase'>Reminders</p> <div className="flex flex-wrap gap-2 w-full">
        {dates?.map((item) => (
          <button
            key={item.id}
